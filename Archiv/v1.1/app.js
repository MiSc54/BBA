...
async function collectBackup(profiles) {
    const [equipment, customEquipment, customExercises] = await Promise.all([
    ...
    for (const p of profiles) {
        const [plans, logs, settings, profileDetails, bodyLog, dashboardConfig] = await Promise.all([
        ...
        profileData[p.id] = { plans, logs, settings, profileDetails, bodyLog, dashboardConfig };
    }
    return {
        version: 2,
        exportedAt: new Date().toISOString(),
        global: { equipment, customEquipment, customExercises },
        profiles,
        profileData,
    };
}
...
/* =========================================================================
   DASHBOARD
   ========================================================================= */
function Dashboard({ logs, plans, onGoToPlans, bodyLog, profileDetails, dashboardConfig }) {
    const stats = useMemo(() => {
        const now = new Date();
    ...
    const age = calcAge(profileDetails?.birthDate);
    const latestBody = sortedBody[sortedBody.length - 1];
    const latestBMI = latestBody ? calcBMI(latestBody.weightKg, heightCm) : null;
    const weightChartData = sortedBody.filter((e) => e.weightKg != null).map((e) => ({ date: formatDateShort(e.date), gewicht: e.weightKg, ziel: goalWeight }));
    const bmiChartData = heightCm ? sortedBody.filter((e) => e.weightKg != null).map((e) => ({ date: formatDateShort(e.date), bmi: Math.round(calcBMI(e.weightKg, heightCm) * 10) / 10 })) : [];
    const visibleMeasurements = (dashboardConfig?.visibleMeasurements || []).filter((id) => sortedBody.some((e) => e.measurements?.[id] != null));
    const measurementChartData = sortedBody.map((e) => {
    ...
    return (React.createElement("div", null,
        ...
        (age != null || latestBody) && (React.createElement("div", { className: "ff-stat-grid", style: { marginTop: 14 } },
            age != null && React.createElement(StatCard, { label: "Alter", value: age, suffix: "Jahre", icon: Users }),
            latestBody?.weightKg != null && React.createElement(StatCard, { label: "Gewicht", value: latestBody.weightKg, suffix: "kg", icon: TrendingUp }),
            latestBMI && React.createElement(StatCard, { label: "BMI", value: latestBMI.toFixed(1), icon: Flame }),
            goalWeight && React.createElement(StatCard, { label: "Zielgewicht", value: goalWeight, suffix: "kg", icon: Calendar }))),
        ...
        dashboardConfig?.showBMI && bmiChartData.length > 0 && (React.createElement("div", { className: "ff-card", style: { marginTop: 20 } },
            React.createElement("p", { className: "ff-eyebrow" }, "K\u00F6rperverlauf"),
            React.createElement("p", { className: "ff-display", style: { fontSize: 22, marginTop: 2, marginBottom: 18 } }, "BMI \u00FCber Zeit"),
            React.createElement("div", { style: { width: "100%", height: 200 } },
                React.createElement(ResponsiveContainer, null,
                    React.createElement(LineChart, { data: bmiChartData, margin: { top: 10, right: 10, left: -10, bottom: 0 } },
                        React.createElement(CartesianGrid, { strokeDasharray: "3 3", stroke: "#222", vertical: false }),
                        React.createElement(XAxis, { dataKey: "date", stroke: "#666", tick: { fontSize: 11, fontFamily: "JetBrains Mono" } }),
                        React.createElement(YAxis, { stroke: "#666", tick: { fontSize: 11, fontFamily: "JetBrains Mono" }, domain: ["auto", "auto"] }),
                        React.createElement(Tooltip, { contentStyle: { background: "#0e0e0e", border: "1px solid #333", borderRadius: 8, fontFamily: "JetBrains Mono", fontSize: 12 }, labelStyle: { color: "#aaa" } }),
                        React.createElement(Line, { type: "monotone", dataKey: "bmi", stroke: "#00fff9", strokeWidth: 2, dot: { r: 3, fill: "#00fff9" } })))))),
...
function PlanEditor({ plan, allExercises, allEquipment, ownedEquipment, settings, currentWeightKg, onSave, onCancel }) {
...
/* =========================================================================
   PLANS LIST VIEW
   ========================================================================= */
function PlansView({ plans, allExercises, allEquipment, ownedEquipment, settings, currentWeightKg, onCreate, onUpdate, onDelete, onStart }) {
    const [editing, setEditing] = useState(null); // null | plan | "new"
    if (editing) {
        return (React.createElement(PlanEditor, { plan: editing === "new" ? null : editing, allExercises: allExercises, allEquipment: allEquipment, ownedEquipment: ownedEquipment, settings: settings, currentWeightKg: currentWeightKg, onCancel: () => setEditing(null), onSave: (plan) => {
    ...
    return (React.createElement("div", null,
        ...
        plans.length === 0 ? (React.createElement("div", { className: "ff-empty", style: { marginTop: 24 } },
            ...
            React.createElement("button", { className: "ff-btn ff-btn-primary", style: { marginTop: 14 }, onClick: () => setEditing("new") },
                React.createElement(Plus, { size: 14 }),
                " Ersten Plan erstellen"))) : (React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 14, marginTop: 22 } }, plans.map((plan) => {
...
function ActiveWorkoutView({ workout, allExercises, onFinish, onCancel }) {
    ...
    return (React.createElement("div", { style: { paddingBottom: rest ? 88 : 0 } },
        ...
        React.createElement("div", { style: { marginTop: 16, maxWidth: 220 } },
            ...
            React.createElement("input", { type: "date", className: "ff-input", value: logDate, onChange: (e) => setLogDate(e.target.value) })),
...
function DiaryEntry({ log, onDelete, onEditDate }) {
    ...
    const commitDate = () => {
        if (dateValue) {
            const original = new Date(log.date);
            const next = new Date(dateValue);
            next.setHours(original.getHours(), original.getMinutes(), original.getSeconds());
        ...
        React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
            React.createElement("div", { style: { cursor: "pointer", flex: 1 }, onClick: () => setOpen((v) => !v) },
                ...
                editingDate ? (React.createElement("div", { style: { display: "flex", gap: 6, marginTop: 6 }, onClick: (e) => e.stopPropagation() },
                    React.createElement("input", { type: "date", className: "ff-input", style: { padding: "6px 10px", fontSize: 12, maxWidth: 160 }, value: dateValue, onChange: (e) => setDateValue(e.target.value), autoFocus: true }),
...
function BodyLogModal({ entry, onClose, onSave }) {
    ...
    return (React.createElement("div", { className: "ff-modal-backdrop", onClick: onClose },
        React.createElement("div", { className: "ff-modal ff-scrollbar", onClick: (e) => e.stopPropagation() },
            ...
            React.createElement("input", { type: "date", className: "ff-input", value: date, onChange: (e) => setDate(e.target.value) }),
            ...
            React.createElement("div", { className: "ff-card" },
                React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 } },
                    React.createElement("div", null,
                        ...
                        React.createElement("input", { type: "date", className: "ff-input", value: detailsForm?.birthDate || "", onChange: setDetailsField("birthDate") })),
                    React.createElement("div", null,
        ...
        React.createElement("div", { style: { marginTop: 32 } },
            ...
            React.createElement("div", { className: "ff-card" },
                React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10 } },
                    ...
                    React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 10, cursor: "pointer" } },
                        React.createElement("div", { className: `ff-checkbox ${dashboardConfig?.showBMI ? "checked" : ""}`, onClick: () => toggleDashboardFlag("showBMI") }, dashboardConfig?.showBMI && React.createElement(Check, { size: 14 })),
                        React.createElement("span", { style: { fontSize: 14 } }, "BMI-Verlauf anzeigen")),
                    React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 10, cursor: "pointer" } },
...
export default function App() {
    ...
    useEffect(() => {
        (async () => {
            const [eq, ceq, ce, existingProfiles] = await Promise.all([
                loadKey(STORAGE_KEYS.equipment, []),
                loadKey(STORAGE_KEYS.customEquipment, []),
                loadKey(STORAGE_KEYS.customExercises, []),
            ...
            const [pl, lg, st, pd, bl, dc] = await Promise.all([
    ...
    const handleFinishWorkout = (exercises, note, logDate) => {
        ...
        const now = new Date();
        const chosenDate = logDate ? new Date(logDate) : now;
    ...
    return (React.createElement("div", { className: "ff-root" },
        ...
        React.createElement("div", { className: "ff-main" }, activeWorkout ? (React.createElement(ActiveWorkoutView, { workout: activeWorkout, allExercises: allExercises, onFinish: handleFinishWorkout, onCancel: () => setActiveWorkout(null) })) : (React.createElement(React.Fragment, null,
            tab === "uebersicht" && (React.createElement(Dashboard, { logs: logs, plans: plans, onGoToPlans: () => setTab("plaene"), bodyLog: bodyLog, profileDetails: profileDetails, dashboardConfig: dashboardConfig })),
            ...
            tab === "plaene" && (React.createElement(PlansView, { plans: plans, allExercises: allExercises, allEquipment: allEquipment, ownedEquipment: equipment, settings: settings, currentWeightKg: currentWeightKg, onCreate: handleCreatePlan, onUpdate: handleUpdatePlan, onDelete: handleDeletePlan, onStart: handleStartWorkout })),
