...
/* =========================================================================
   BERNDS BODY APP — Quellcode-Übersicht für Einsteiger
   =========================================================================
   Diese Datei enthält die GESAMTE App als eine einzige React-Komponenten-
   Datei. "React" ist ein JavaScript-Werkzeug, mit dem man Benutzeroberflächen
   aus kleinen, wiederverwendbaren Bausteinen ("Komponenten") zusammenbaut.
   Jede Komponente ist eine Funktion, die beschreibt, was auf dem Bildschirm
   erscheinen soll (der Teil mit "return (...)" — das ist HTML-ähnlicher
   Code namens JSX).

   GROBER AUFBAU DER DATEI (von oben nach unten):
   1. Logo & Versionsnummer (ganz oben)
   2. Feste Datenlisten: Geräte (EQUIPMENT), Übungen (EXERCISE_LIBRARY),
      Muskelgruppen, Wochentage usw. — HIER findest du die Inhalte, die am
      ehesten manuell angepasst werden wollen (z.B. neue Übung ergänzen).
   3. Speicher-Funktionen (loadKey/saveKey/collectBackup/...) — kümmern
      sich ums dauerhafte Speichern der Daten auf dem Gerät.
   4. Hilfsfunktionen (Berechnungen wie BMI, Kalorien, Datum formatieren).
   5. GlobalStyles — das komplette Design/Aussehen (Farben, Schriften) als
      CSS-Text. Zum Farben ändern: hier suchen.
   6. Viele kleine/mittlere Komponenten, jede zuständig für einen
      Bildschirm-Bereich (z.B. Dashboard, EquipmentView, PlanEditor...).
   7. Ganz unten: "App" — die Hauptkomponente, die alles zusammenhält.

   WIE FINDE ICH ETWAS?
   Jede Komponente/Funktion beginnt mit "function NameDerKomponente(...)"
   oder "const NAME = ...". Du kannst mit Strg+F (bzw. Cmd+F) nach dem
   Namen suchen, den du in der App gesehen hast, oder nach den Kommentaren
   in Großbuchstaben wie "GERÄTE-TAB" oder "ÜBERSICHT-TAB" suchen — die
   markieren jeweils den Beginn eines neuen Bereichs.

   VORSICHT bei Änderungen:
   - IDs (id: "...") von Geräten/Übungen NICHT verändern, wenn schon
     Trainingspläne oder Tagebuch-Einträge existieren, die darauf verweisen.
   - Geschweifte Klammern { } und eckige Klammern [ ] müssen immer paarweise
     vorkommen — beim Bearbeiten am besten nur einzelne Werte (Texte,
     Zahlen, Farben) ändern, nicht die Struktur drumherum.
   ========================================================================= */
...
function buildWorkoutsCSV(logs) {
...
function buildBodyLogCSV(bodyLog, heightCm) {
...
function bestValueForSet(set, trackingType) {
    if (trackingType === "distance")
...
function detectPRs(newExercises, previousLogs, allExercises) {
    const prs = [];
...
/* =========================================================================
   ...
   ========================================================================= */
function ExercisesView({ allExercises, allEquipment, ownedEquipment, logs, onAddCustom, favoriteExercises, onToggleFavoriteExercise, favoriteEquipment }) {
    const [search, setSearch] = useState("");
...
/* =========================================================================
   ...
   ========================================================================= */
function PlanEditor({ plan, initialTemplate, allExercises, allEquipment, ownedEquipment, settings, currentWeightKg, favoriteExercises, onSave, onCancel }) {
...
/* =========================================================================
   ...
   ========================================================================= */
function PlansView({ plans, allExercises, allEquipment, ownedEquipment, settings, currentWeightKg, favoriteExercises, favoritePlans, onToggleFavoritePlan, onCreate, onUpdate, onDelete, onClone, onStart, hasActiveWorkout }) {
...
function TemplatePickerModal({ allExercises, onPick, onClose }) {
    return (React.createElement("div", { className: "ff-modal-backdrop", onClick: onClose },
        React.createElement("div", { className: "ff-modal ff-scrollbar", onClick: (e) => e.stopPropagation() },
            React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 } },
...
function ActiveWorkoutView({ workout, allExercises, onUpdate, onFinish, onCancel }) {
...
function DiaryView({ logs, onDelete, onEditDate, onGoToPlans, bodyLog, heightCm, onAddBodyEntry, onUpdateBodyEntry, onDeleteBodyEntry, activeWorkout, onResumeWorkout, onCancelWorkout }) {
...
function SettingsView({ settings, onSaveSettings, profiles, activeProfileId, onCreateProfile, onRenameProfile, onDeleteProfile, onSwitchProfile, onExportBackup, onImportBackup, profileDetails, onSaveProfileDetails, dashboardConfig, onSaveDashboardConfig, onExportWorkoutsCSV, onExportBodyLogCSV, }) {
